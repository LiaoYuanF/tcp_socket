...
def handle_upload(connection,address):
    while True:
        try:
            ...
            connection.close()
        break
def handle_download(new_socket):
         fileinfo_size=struct.calcsize('128sl')
         buf = connection.recv(fileinfo_size)
         ...
         if os.path.isfile(filepath):
            left_str = "******** "\
               + "file download" + \
               " has left ********"
            new_socket.sendall(("#!downlod#!" + left_str).encode("utf-8"))
    ...
    try:
        while True:
            ...
            if req_type:  # 如果不为真，则说明客户端已断开
                ...
                elif req_type == "4":  # 上传文件
                    print("开始处理上传文件请求")
                    handle_upload(new_socket,addr)
                elif req_type == "5":  # 下载文件
                    print("开始处理下载文件请求")
                    handle_download(new_socket,addr)
            else:
                break
    except Exception as ret:
